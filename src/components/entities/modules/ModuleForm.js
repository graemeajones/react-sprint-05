...
export default function ModuleForm({ onCancel, onSubmit, initialRecord=emptyRecord }) {
  ...
  const validation = {
    ...
    errorMessage: {
      ...
      ModuleLeaderID: "This message should not appear!",
  ...
  const conformance = {
    js2html: {
      ...
      ModuleLevel: (level) => level,
      ModuleYearID: (id) => id === null ? 0 : id,
      ModuleLeaderID: (id) => id === null ? 0 : id,
      ModuleImageURL: (url) => url
    },
    html2js: {
      ModuleName: (name) => name,
      ModuleCode: (code) => code,
      ModuleLevel: (level) => parseInt(level),
      ModuleYearID: (id) => parseInt(id) === 0 ? null : parseInt(id),
      ModuleLeaderID: (id) => parseInt(id) === 0 ? null : parseInt(id),
      ModuleImageURL: (url) => url
    }
  };
